...
const ProductRating = ({ queryConfig }: Props) => {
  ...
  const [hiddenRating, setHiddenRating] = useState(true)
  return (
    <div
      className='flex flex-col relative mobile:w-full '
      onMouseEnter={() => setHiddenRating(false)}
      onMouseLeave={() => setHiddenRating(true)}
    >
      <button className='dark:bg-[#24242c] dark:text-white dark:border-none border hover:opacity-90 rounded-lg p-2 '>
        {t('rating filter')}
      </button>
      <ul
        className={`overflow-hidden mobile:z-10 absolute flex flex-col top-[110%] bg-white dark:text-white dark:border-none rounded-lg w-[130%] border ${
          hiddenRating ? 'hidden' : 'block'
        }`}
      >
        {Array(5)
          .fill(0)
          .map((_, index) => (
            <button
              ...
              className='py-3 pl-2 dark:bg-[#24242c] dark:hover:bg-text-color hover:bg-slate-50'
              key={index}
            >
